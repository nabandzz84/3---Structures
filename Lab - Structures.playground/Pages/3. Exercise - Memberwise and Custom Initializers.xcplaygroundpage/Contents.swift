...
prinnt ("My favortie book is called \(favoriteBooks2.title)")
/*:
...
struct height{
    ...
    intit(heightInInches: Double){
        self.heightInInches = heightInInches
        self.heightInCentimeters = heightInCentimeters * 2.54
    }
    
    intit(heightInCentimeters: Double){
...
//:  Now create a variable instance of `Height` called `myHeight` and initialize it with your own height. Verify that both `heightInInches` and `heightInCentimeters` are accurate.
    var mysHeight = Height (Height(heightInCentimers: 178)
